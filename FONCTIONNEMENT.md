...
- **Upload** : `tools.uploadAsset(file)` → `PUT /api/asset` → retourne `user://filename`
- **Suppression** : `tools.deleteAsset(filename)` → `DELETE /api/asset?filename=<name>` → `{ ok: true }`
- **Liste** : `GET /api/assets` (auth requise) → `{ files: [{ name, path, mimeType }] }`
- **Résolution** : `api.resolveAsset('user://filename')` → `GET /api/asset?filename=...` → URL signée ou contenu direct

Les assets sont référencés dans la config par le préfixe `user://`, résolu au moment du rendu côté overlay.

### Gestionnaire dans le dashboard

La section **Outils → Mes fichiers** (`id="section-assets"`, rendu par `renderAssets()`) liste tous les fichiers uploadés avec aperçu (miniature ou lecteur audio), et permet d'uploader ou supprimer des fichiers.

### Pattern admin — createAssetField

Dans les mods, ne jamais gérer l'upload manuellement. Utiliser `tools.createAssetField({ type, value, onChange })` qui retourne un `<div class="asset-field">` contenant :
- un `<div class="upload-row">` avec un `<select>` (assets intégrés + fichiers uploadés) et un bouton d'upload
- un élément de prévisualisation automatique : `<img class="asset-img-preview">` (gif/image) ou `<audio class="asset-sound-preview">` (son)

```js
// Dans le template HTML du mod :
// <div id="mon-asset-field"></div>
